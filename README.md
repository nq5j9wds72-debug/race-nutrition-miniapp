...
# 18. Что будет храниться в БД в MVP

Когда будет подключён Render Postgres, в первой версии памяти продукта планируется хранить только:

## 18.1. `users`

Зачем нужна таблица:

- помнить подтверждённых Telegram-пользователей
...
- показывать пользователю последние расчёты
- дать первую прямую пользу от БД

## 18.3. Что сознательно не добавляем сейчас
